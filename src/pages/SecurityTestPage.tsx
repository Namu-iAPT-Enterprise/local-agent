  ...
  return [
    // ── 시나리오 1 ─────────────────────────────────────────────────────────────
    {
      ...
      steps: [
        ...
        { label: '정리: 역할 삭제', actor: 'admin', method: 'DELETE', path: `/api/role/define/${TARGET_B}`, expected: [200, 204, 404], role: 'info' },
        { label: '정리: 역할 삭제', actor: 'admin', method: 'DELETE', path: `/api/role/define/${LIMITED}`, expected: [200, 204, 404], role: 'info' },
    ...
    // ── 시나리오 2 ─────────────────────────────────────────────────────────────
    {
      ...
      steps: [
        ...
        { label: '공격자에게 역할 배정', actor: 'admin', method: 'POST', path: `/api/role/assign`, body: { userId: '__ATTACKER__', roleId: LIMITED }, expected: [200, 201], role: 'info' },
        {
          ...
          actor: 'attacker',
          method: 'PATCH',
          path: `/api/role/define/${OTHER_ROLE}`,
          body: { displayName: '해킹됨', permissionTagIds: ['CHAT_MESSAGE', 'FILE_UPLOAD'] },
          expected: 403,
          role: 'should-block',
          note: '자기 팀(TEAM)에 대한 권한만 있으므로 차단되어야 함',
        },
        { label: '정리: 역할들 삭제', actor: 'admin', method: 'DELETE', path: `/api/role/define/${OTHER_ROLE}`, expected: [200, 204, 404], role: 'info' },
        { label: '정리: 역할 삭제', actor: 'admin', method: 'DELETE', path: `/api/role/define/${LIMITED}`, expected: [200, 204, 404], role: 'info' },
        { label: '정리: 팀 삭제', actor: 'admin', method: 'DELETE', path: `/api/team/${TEAM_B}`, expected: [200, 204, 404], role: 'info' },
        { label: '정리: 팀 삭제', actor: 'admin', method: 'DELETE', path: `/api/team/${TEAM}`, expected: [200, 204, 404], role: 'info' },
      ],
    },
    ...
    // ── 시나리오 6 ─────────────────────────────────────────────────────────────
    {
      ...
      steps: [
        { label: '팀 생성', actor: 'admin', method: 'POST', path: `/api/team`, body: { teamId: TEAM, name: '공격 테스트팀', color: '#e74c3c' }, expected: [200, 201, 409], role: 'info' },
        { label: '공격자 역할 생성 (FILE_UPLOAD 포함)', actor: 'admin', method: 'POST', path: `/api/role/define`, body: { roleId: LIMITED, teamId: TEAM, displayName: '제한된 역할', permissionTagIds: ['CHAT_MESSAGE', 'FILE_UPLOAD'], manageableByRoleIds: [] }, expected: [200, 201, 409], role: 'info' },
        { label: '공격자에게 역할 배정', actor: 'admin', method: 'POST', path: `/api/role/assign`, body: { userId: '__ATTACKER__', roleId: LIMITED }, expected: [200, 201], role: 'info' },
        {
        ...
        { label: '관리자: 역할에서 FILE_UPLOAD 태그 제거', actor: 'admin', method: 'PATCH', path: `/api/role/define/${LIMITED}`, body: { permissionTagIds: ['CHAT_MESSAGE'] }, expected: [200, 204], role: 'info', note: '이 시점에 캐시가 자동 evict되어야 함' },
        {

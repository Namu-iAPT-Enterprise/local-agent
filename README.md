...
VLAN 10 (WEB 운영 영역)의 단말에 설치/실행되며, 백엔드 서버들과 직접 통신하지 않고 **BFF (Backend-for-Frontend) 또는 API Gateway**를 단일 진입점으로 사용합니다.
...
NAMU LA (Electron App)
    ...
    ▼
BFF Server (VLAN 10) / API Gateway   ← 단일 진입점
    │
    ├──► [VLAN 20] Security Server (OAuth 2.0) 인증/JWKS 제공
    ├──► [VLAN 20] Role Server (OPA) AuthZ 및 역할 API (/api/role)
    ├──► [VLAN 20] Main Agent AI 채팅/LLM 연동
    └──► [VLAN 30] Storage/RAG 등 (Zero Trust, JWT 직접 검증)
```
...
② 기본 UI 표시 (채팅 화면 진입)
③ GET /api/role/profile → permissionRoles + enabledFeatures + allowedApis 수신
...
Role Server(또는 연동된 BFF/Gateway)로부터 수신한 `enabledFeatures` 배열을 기반으로 UI를 제어합니다.  
...
| `src/api/gateway.ts` | `GET /api/role/profile` | 전체 권한 정보 조회 (UI 초기화용) |
...
- 게이트웨이는 이제 JWT Passthrough를 수행하며, 백엔드는 JWKS를 통해 JWT를 직접 검증(Zero Trust)합니다.
